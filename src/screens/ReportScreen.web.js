...
const REPORT_CHART_WIDTH = Math.min(980, Math.max(width - 140, 280));
    ...
    try {
      if (careUserId) {
        ...
        if (!acc || !acc.token) {
          Alert.alert(
            '提示',
            '无法使用关怀数据：未找到该关怀账号，或当前浏览器登录已覆盖添加关怀时的账号。请先切回添加关怀的账号，或使用两个独立的浏览器配置/隐私窗口分别登录。',
          );
            ...
            <Card style={styles.card}>
              <Card.Content>
                ...
                <StackedBarChart
                  ...
                  width={Math.max(300, REPORT_CHART_WIDTH - 8)}
                  height={220}
                  fromZero
                  segments={4}
                  yAxisSuffix="h"
            ...
            {normalizedAssistantAdvice.length > 0 && !report?.careMode ? (
              ...
              <>
                <Paragraph style={styles.adviceHint}>
                  {report?.careMode
                    ? '以下为基于对方快照的规则简要提示。同一浏览器多开会共用本地存储、易覆盖关怀凭证：请改用不同浏览器配置或无痕窗口分开登录后再试；若对方未同步体征快照则会显示为 0。'
